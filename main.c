...
//                 -----------
//    Chip Select -|1		8|- VCC
//    Data Output -|2 		7|- Hold/Reset
//  Write Protect -|3 	 	6|- CLK
//            GND -|4	  	5|- Data Input
// 			       -----------
...
//         PA0 -|6       27|- PB4
//         PA1 -|7       26|- PB3
//         PA2 -|8       25|- PA15 SPEAKER
//  25Q32  PA3 -|9       24|- PA14
//         PA4 -|10      23|- PA13
//  25Q32  PA5 -|11      22|- PA12
//  25Q32  PA6 -|12      21|- PA11
//  25Q32  PA7 -|13      20|- PA10 (Reserved for RXD on BO230XS)
//         PB0 -|14      19|- PA9  (Reserved for TXD on BO230XS)
//         PB1 -|15      18|- PA8
//         VSS -|16      17|- VDD
//               ----------
...
// PA3: CSn (but can be any GPIO pin)
// PA5: SPI1_SCK  (AFR)
// PA6: SPI1_MISO (AFR)
// PA7: SPI1_MOSI (AFR)
// PA15: TIM2_CH1 to speaker (AFR) -> Needs pin because of PWM output
...
void Timer22_Init(void) 
{
	...
	// Set up timer
	RCC->APB2ENR |= BIT5;  // turn on clock for timer22 (UM: page 181)
	TIM22->ARR = F_CPU/(22050L)-1;
	NVIC->ISER[0] |= BIT22;  // enable timer 22 interrupts in the NVIC (p240)
...
// The following function is associated with the TIM22 interrupt 
...
void TIM22_Handler(void) 
{
	...
	{  
		...
		else
		{
			...
			TIM21->CCR1=SPIWrite(0x00);
			playcnt--;
		}
	}
}

// Timer 21, more specifically channel 1 of timer 21, is used to produce an 8-bit PWM signal
// used as DAC to produce the sound.
void Timer21_Init(void)
{
	GPIOA->OSPEEDR |= BIT4; // MEDIUM SPEED
	GPIOA->OTYPER &= ~BIT2; // Push-pull
	GPIOA->MODER &= ~(BIT5 | BIT4); // AF-Mode
	GPIOA->MODER |= BIT5; // AF-Mode
	GPIOA->AFR[0] &= ~(BIT11 | BIT10 | BIT9 | BIT8); // AF0 selected (check table 16 in page 43 of "en.DM00108219.pdf")

	RCC->APB2ENR |= BIT2;  // turn on clock for timer21 (UM: page 176)
	TIM21->ARR = 255;
	...
	// Enable PWM in channel 1 of Timer 21
	TIM21->CCMR1|=BIT6|BIT5; // PWM mode 1 ([6..4]=110)
	TIM21->CCMR1|=BIT3; // OC1PE=1
	TIM21->CCER|=BIT0; // Bit 0 CC1E: Capture/Compare 1 output enable.
